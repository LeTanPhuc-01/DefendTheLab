...
class InitialMenu extends Phaser.Scene {
    ...
    goBackToGame() {
        ...
        // Switch controller to GAME mode (OCR Canvas)
        if (window.setControllerMode) {
            console.log("InitialMenu: Calling setControllerMode('GAME')");
            window.setControllerMode('GAME');
        } else {
            console.error("InitialMenu: window.setControllerMode is NOT defined!");
        }
